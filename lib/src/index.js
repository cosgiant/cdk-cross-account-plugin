...
class CrossAccountCredentialProvider {
    ...
    canProvideCredentials(accountId) {
        ...
        if ((0, fs_1.existsSync)('cdk.json')) {
            ...
            this.crossAccountConfig = (0, dot_prop_1.get)(this.cdkConfig, `crossAccountConfig`, undefined);
    ...
    getProvider(accountId, mode) {
        ...
        let config = (0, dot_prop_1.get)(this.crossAccountConfig, accountId);
    ...
    resolveWithProfile(profileName, targetAccount) {
        ...
        if (this.pluginConfig.has(`credentialCache.${profileName}`)) {
            ...
            if (now < expires) {
                let timeRemaining = (0, date_fns_1.formatDistance)(now, expires);
        ...
        if (profile.sso_start_url) {
            ...
            let ssoCacheDirectory = (0, path_1.resolve)((0, os_1.homedir)(), '.aws', 'sso', 'cache');
            ...
            let ssoToken = (0, fs_jetpack_1.find)(ssoCacheDirectory, { matching: ['*.json', '!botocore*'], recursive: false })
